export const projects = [
  {
    id: 1,
    title: "DarkProbe",
    description: "A production-grade modular web vulnerability scanner built in Python. Detects critical vulnerabilities and generates structured professional reports. Designed as a lightweight alternative to enterprise security tools.",
    category: "Cybersecurity",
    techStack: ["Python", "Security Analysis", "Automation"],
    link: "https://github.com/abhinaavchauhan/DarkProbe",
    image: "/projects/darkprobe.png"
  },
  {
    id: 2,
    title: "ScreenFusion",
    description: "A scalable web system that merges multiple smartphones into a unified virtual display. Enables synchronized real-time viewing across devices.",
    category: "System Design",
    techStack: ["React", "WebSockets", "Node.js"],
    link: "https://github.com/abhinaavchauhan/ScreenFusion",
    image: "/projects/screenfusion.png"
  },
  {
    id: 3,
    title: "SwiftShare",
    description: "A premium Android application for high-speed offline file transfer using Bluetooth RFCOMM sockets. Built with MVVM architecture and Material Design principles.",
    category: "Mobile",
    techStack: ["Android", "Java", "Bluetooth RFCOMM", "MVVM"],
    link: "https://github.com/abhinaavchauhan/SwiftShare",
    image: "/projects/swiftshare.png"
  },
  {
    id: 4,
    title: "Bookstore",
    description: "A full-stack e-commerce platform with Node.js/Express backend and NestJS-based server-side rendered frontend.",
    category: "Full Stack",
    techStack: ["Node.js", "Express.js", "NestJS", "E-commerce"],
    link: "https://github.com/abhinaavchauhan",
    image: "/projects/bookstore.png"
  },
  {
    id: 5,
    title: "Veda",
    description: "A Python-based virtual assistant with voice command support, real-time data retrieval, and system automation capabilities.",
    category: "Artificial Intelligence",
    techStack: ["Python", "Automation", "Voice AI"],
    link: "https://github.com/abhinaavchauhan",
    image: "/projects/veda.png"
  },
  {
    id: 6,
    title: "Expense Tracker",
    description: "A full-stack expense management system with Node.js backend and Jetpack Compose Android frontend. Supports analytics and expense sharing.",
    category: "Full Stack",
    techStack: ["Node.js", "Jetpack Compose", "Android", "Analytics"],
    link: "https://github.com/abhinaavchauhan",
    image: "/projects/expense.png"
  }
];
